...
import AuctionTable from '@/app/components/AuctionTable';

export default function BridgeHandLayout(props: {
  auction: ValueAndDetails;
  board: string;
  deal: string;
  dealer: string;
  declarer: string;
  play: ValueAndDetails;
  vulnerable: string;
}) {
  const vulnerability = {
  ...
  return (
    ...
    <div className='relative m-2 flex size-[500px] flex-col items-center justify-center rounded-lg border-2 border-black p-4'>
      ...
      <div className='absolute left-1 top-1 rounded-md px-3 py-2 shadow-md'>
        ...
        <p>
          Vul: <span className='font-bold'>{props.vulnerable}</span>
        </p>
      </div>
      ...
      <div className='absolute flex size-32 flex-col items-center justify-center border-4 border-white bg-gray-800 text-lg font-bold text-white'>
        ...
        <div className='absolute text-xl font-extrabold'>{props.board}</div>
        <div
        ...
        >
          {/*<div className={'px-2 py-1 bg-white'} >*/}W{/*</div>*/}
        </div>
        <div
