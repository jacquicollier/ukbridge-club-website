...
/** not-t@type {!BigUint64Array} */
    ...
    function findWasmBinary() {
      if (Module['locateFile']) {
        return locateFile('dds.wasm');
      }
      // Use bundler-friendly `new URL(..., import.meta.url)` pattern; works in browsers too.
      return new URL(window.location + '/dds.wasm', import.meta.url).href;
    }
...
(() => {
  // Create a small, never-async wrapper around createDDSModule which
  // checks for callers incorrectly using it with `new`.
  var real_createDDSModule = createDDSModule;
  createDDSModule = function (arg) {
    if (new.target)
      throw new Error(
        'createDDSModule() should not be called with `new createDDSModule()`',
      );
    return real_createDDSModule(arg);
  };
})();
export default createDDSModule;
